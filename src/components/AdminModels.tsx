...
import { Plus, Trash2, Save, Code, Check, Upload, Globe, Lock, Eye, EyeOff, LayoutPanelLeft, LayoutGrid, Settings, X, Edit2, Sparkles, Camera, Shirt, MapPin, Wand2, User, Image, Palette, Zap, ChevronDown, Download, Play } from "lucide-react"
...
export function AdminModels({ onModuleChange }: AdminModelsProps) {
        ...
        reader.onload = (event) => {
            ...
            if (content) {
                setEditingTemplate(prev => {
                    const next = { ...prev };
                    if (!next.name) next.name = file.name.replace(".json", "");
                    return next;
                });
    ...
    const handleSave = async () => {
        if (!editingTemplate?.name || !editingTemplate?.rawJson) {
    ...
    const handleDelete = async (id: string) => {
        if (!confirm("Tem certeza que deseja deletar este template global?")) return
        try {
    ...
    const handleDeleteCategory = async (id: string) => {
        if (!confirm("Confirmar exclusão da categoria?")) return
        try {
                        ...
                        {isLoading ? (
                            ...
                            <div className="col-span-full py-20 bg-white/[0.02] border border-dashed border-white/10 rounded-3xl flex flex-col items-center justify-center text-center px-6">
                                ...
                                <p className="text-xs text-muted-foreground max-w-xs uppercase tracking-widest font-black opacity-50">
                                    Comece criando o seu primeiro template de workflow para o SaaS.
                                </p>
                            </div>
                        ) : (
                            templates.map(t => (
                                <div key={t.id} className="bg-[#0c0c0e] border border-white/10 rounded-2xl p-6 transition-all hover:border-primary/30 group relative">
                    ...
                    <div className="bg-[#0c0c0e] border border-white/10 rounded-3xl p-8 max-w-3xl mx-auto shadow-2xl">
                        ...
                        <div className="space-y-6">
                            <div className="grid grid-cols-2 gap-4">
                                ...
                                <div className="space-y-1.5 flex-1 text-left relative">
                                    ...
                                    {isCatDropdownOpen && (
                                        <div className="absolute top-full left-0 mt-2 w-full bg-[#111114] border border-white/10 rounded-xl shadow-2xl p-2 z-[200] max-h-48 overflow-y-auto no-scrollbar animate-in fade-in slide-in-from-top-2">
                                            <button
                                                onClick={() => { setEditingTemplate({ ...editingTemplate!, categoryId: "" }); setIsCatDropdownOpen(false); }}
                                            ...
                                            {categories.map(cat => (
                                                <button
                                                    key={cat.id}
                                                    onClick={() => { setEditingTemplate({ ...editingTemplate!, categoryId: cat.id }); setIsCatDropdownOpen(false); }}
                                                    className={cn(
                                                ...
                                                <div key={m.key} className="space-y-1.5 relative">
                                                    ...
                                                    {openMappingDropdown === m.key && (
                                                        <div className="absolute top-full left-0 mt-1 w-full bg-[#16161a] border border-white/10 rounded-lg shadow-xl p-1 z-[210] max-h-40 overflow-y-auto no-scrollbar">
                                                            <button
                                                                //@ts-ignore
                                                                onClick={() => { setEditingTemplate({ ...editingTemplate!, mapping: { ...editingTemplate?.mapping!, [m.key]: "" } }); setOpenMappingDropdown(null); }}
                                                            ...
                                                            {Object.entries(parsedNodes).map(([id, node]) => (
                                                                <button
                                                                    key={id}
                                                                    //@ts-ignore
                                                                    onClick={() => { setEditingTemplate({ ...editingTemplate!, mapping: { ...editingTemplate?.mapping!, [m.key]: id } }); setOpenMappingDropdown(null); }}
                                                                    className={cn(
