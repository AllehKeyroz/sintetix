...
import { Sparkles, Camera, Shirt, MapPin, Wand2, Download, Loader2, Settings, X, Activity, Trash2, CheckCircle2, MessageSquare, Plus, Maximize2, User, Image, Palette, Zap, LayoutGrid, Copy, Check } from "lucide-react"
...
export function AIGeneratorStudio({ influencerId, isAdminMode = false }: AIStudioProps) {
    ...
    const [history, setHistory] = useState<{ id: string, url: string | null, urls?: string[], prompt: string, model: string, size: string, isLoading: boolean, error: string | null, cost?: number, duration?: number }[]>([])
    ...
    const fetchQuota = async () => {
        try {
            const res = await fetch("/api/nordy/quota");
            if (res.ok) {
    ...
    const handleCancelGeneration = (id: string) => {
        if (pollIntervalsRef.current[id]) {
            ...
            // Opcional: Aqui poderíamos chamar uma API do Nordy para realmente cancelar o job remotamente
            // se eles suportarem (ex: /api/nordy/cancel?jobId=...)
    ...
    const handleGenerate = async () => {
        ...
        if (!prompt.trim() && !hasImages && !hasParams) {
            setErrorMsg("Digite um prompt ou forneça uma imagem/parâmetro.");
            return;
        }

        if (currentWorkflow.mapping?.promptRequired && !prompt.trim()) {
        ...
        const currentPrompt = prompt || "Geração técnica"
        ...
        const newItem = {
            ...
            url: null,
            urls: [],
            prompt: prompt || "Uso de Ferramenta (Sem prompt)",
            model: currentModel,
            size: currentSize,
            isLoading: true,
            error: null,
        ...
        try {
            ...
            for (const [nodeId, data] of Object.entries(imageInputs)) {
                if (data.file) {
                    ...
                    const upRes = await fetch("/api/nordy/upload", { method: "POST", body: formData });
            ...
            if (mapping.negativePromptNodeId && workflowJson[mapping.negativePromptNodeId]) {
                ...
                if (node.inputs) {
                    ...
                    negFields.forEach(f => {
                        if (node.inputs.hasOwnProperty(f)) {
                            node.inputs[f] = ""; // Por enquanto vazio, ou poderíamos ter um campo no UI
                        }
                    });
                }
            }
            ...
            const res = await fetch("/api/nordy/generate", {
                method: "POST",
            ...
            const poll = setInterval(async () => {
                ...
                try {
                    const stRes = await fetch(`/api/nordy/status?jobId=${jobId}`);
                    ...
                    if (status === "done" || status === "completed" || status === "success") {
                        ...
                        // 5. Coletar TODAS as URLs de saída
    ...
    return (
        <div className="flex-1 flex flex-col bg-background relative overflow-hidden">
            <div className="px-11 py-6 flex items-center justify-between border-b border-white/5 bg-black/20 backdrop-blur-md sticky top-0 z-40">
                    ...
                    {dynamicCategories.length > 0 && (
                        dynamicCategories.map(cat => {
                            ...
                            return (
                                ...
                                >
                                    <Icon className={cn("w-3 h-3", activeTab === cat.id ? "text-white" : "text-primary/60")} />
                                    {cat.name}
                                </button>
                            )
                        })
                ...
                <div className="flex items-center gap-3">
                    <div className="flex items-center bg-black/40 border border-white/10 rounded-xl p-1 shadow-inner relative">
                        {/* Seletor Customizado de Modelos */}
                        <div className="relative">
                            <button
                                onClick={() => setIsWorkflowDropdownOpen(!isWorkflowDropdownOpen)}
                                onBlur={() => setTimeout(() => setIsWorkflowDropdownOpen(false), 200)}
                                className="flex items-center gap-2 px-3 py-1.5 min-w-[140px] max-w-[200px] hover:bg-white/5 transition-colors rounded-lg group"
                            >
                                <Zap className={cn("w-3 h-3 transition-colors", selectedWorkflowId ? "text-primary fill-primary/20" : "text-muted-foreground")} />
                                <span className="bg-transparent text-[9px] font-black uppercase text-white outline-none cursor-pointer truncate flex-1 text-left">
                                    {workflows.find(w => w.id === selectedWorkflowId)?.name || "Selecionar Modelo"}
                                </span>
                                <Plus className={cn("w-2.5 h-2.5 text-muted-foreground transition-transform duration-300", isWorkflowDropdownOpen && "rotate-45")} />
                            </button>

                            {isWorkflowDropdownOpen && (
                                <div className="absolute top-full left-0 mt-2 w-56 bg-[#0c0c0e] border border-white/10 rounded-xl shadow-2xl p-2 z-[100] animate-in fade-in slide-in-from-top-2 duration-300 backdrop-blur-xl">
                                    <div className="text-[7px] font-black uppercase tracking-widest text-muted-foreground px-3 py-2 border-b border-white/5 mb-1">Modelos Disponíveis</div>
                                    <div className="max-h-64 overflow-y-auto no-scrollbar">
                                        {workflows.filter(w => w.categoryId === activeTab || w.type === activeTab).length === 0 ? (
                                            <div className="px-3 py-4 text-[9px] text-muted-foreground italic text-center">Nenhum modelo nesta aba</div>
                                        ) : (
                                            workflows.filter(w => w.categoryId === activeTab || w.type === activeTab).map(w => (
                                                <button
                                                    key={w.id}
                                                    onClick={() => {
            ...
            <div className="flex-1 flex min-h-0 overflow-hidden">
                ...
                <div className="flex-1 flex flex-col gap-6 pl-8 relative min-h-0 overflow-hidden">
                        ...
                        {history.length === 0 ? (
                            ...
                            history.map((item) => (
                                <div key={item.id} className="flex flex-col md:flex-row bg-white/[0.01] border border-white/5 rounded-[2.5rem] overflow-hidden group hover:border-white/10 transition-all h-fit md:h-[450px]">
                                    ...
                                    <div className="w-full md:w-1/2 aspect-square md:aspect-auto h-full bg-black/40 relative flex items-center justify-center overflow-hidden border-r border-white/5">
                                        {item.isLoading ? (
                                            <div className="flex flex-col items-center gap-3">
                                                ...
                                                <button onClick={() => handleCancelGeneration(item.id)} className="mt-2 px-4 py-1.5 rounded-full bg-red-500/10 hover:bg-red-500/20 text-red-500 hover:text-red-400 text-[9px] font-black uppercase tracking-widest transition-all">Cancelar Execução</button>
                                            </div>
                                        ) : item.error ? (
                                            ...
                                            <div className={cn("relative w-full h-full group/img overflow-y-auto no-scrollbar", (item.urls && item.urls.length > 1) ? "p-4 grid grid-cols-2 gap-3 content-start" : "flex items-center justify-center")}>
                                                {(item.urls && item.urls.length > 0) ? (
                                                    item.urls.map((u: string, idx: number) => (
                                                        <div key={idx} className={cn("relative overflow-hidden rounded-[1.5rem] border border-white/5 bg-black/20 group/subimg shadow-2xl transition-all hover:border-primary/30", (item.urls && item.urls.length === 1) ? "w-full h-full" : "aspect-square")}>
                                                            {u.toLowerCase().endsWith(".mp4") ? (
                                                                <video src={u} controls className="w-full h-full object-contain" />
                                                            ) : (
                                                    ...
                                                    <div className="relative w-full h-full">
                                                        <img src={item.url} className="w-full h-full object-contain transition-all duration-1000 group-hover:scale-105" />
                                                        <div className="absolute inset-0 bg-black/40 opacity-0 group:opacity-100 transition-all flex items-center justify-center gap-3 backdrop-blur-[2px]">
                                    ...
                                    <div className="flex-1 p-8 flex flex-col justify-center bg-black/20">
                                        <div className="mb-6">
                                            <div className="flex items-center justify-between mb-4">
                                                <span className="text-[9px] font-black uppercase tracking-[0.2em] text-primary/80">Configurações</span>
                                                <div className="flex gap-2">
                                                    <span className="px-3 py-1 rounded-full bg-white/5 border border-white/10 text-[8px] font-black uppercase text-muted-foreground">{item.model}</span>
                    ...
                    <div className="absolute bottom-0 left-8 right-2 bg-background pt-4 pb-2 z-10 before:absolute before:inset-x-0 before:bottom-full before:h-12 before:bg-gradient-to-t before:from-background before:to-transparent before:pointer-events-none">
                        <div className="flex gap-4 mb-4 overflow-x-auto no-scrollbar pb-2">
                            ...
                            {(() => {
                                ...
                                return allInputs.map((input) => (
                                    <label key={input.nodeId} className="shrink-0 cursor-pointer group">
                                        <div className={cn("w-24 h-24 rounded-2xl border-2 border-dashed flex flex-col items-center justify-center gap-1.5 transition-all relative overflow-hidden group/img-input", imageInputs[input.nodeId]?.file || imageInputs[input.nodeId]?.url ? "border-primary/50 bg-primary/5" : "border-white/10 hover:border-primary/30 bg-white/[0.02]")}>
                                            {imageInputs[input.nodeId]?.url ? (
                                                <>
                                                    <img src={imageInputs[input.nodeId].url!} className="w-full h-full object-cover rounded-xl" />
                                                    <div className="absolute inset-0 bg-black/60 opacity-0 group-hover/img-input:opacity-100 transition-all flex items-center justify-center backdrop-blur-[2px]">
                                                        <button
                                                            ...
                                                            className="p-1.5 bg-red-500 rounded-lg text-white hover:bg-red-600 transition-all shadow-xl"
                                                        >
                                                ...
                                                <>
                                                    ...
                                                    <span className="text-[7px] font-black uppercase tracking-widest text-muted-foreground text-center px-2 leading-tight">{input.label}</span>
                                                </>
                                            )}
                                        </div>
                                        <input type="file" className="hidden" accept="image/*" onChange={(e) => { const file = e.target.files?.[0]; if (file) { setImageInputs(prev => ({ ...prev, [input.nodeId]: { file, url: URL.createObjectURL(file) } })); } }} />
                                    </label>
                                ));
                            })()}
                        </div>
                        ...
                        {workflows.find(w => w.id === selectedWorkflowId)?.mapping?.parameterInputs && (workflows.find(w => w.id === selectedWorkflowId)?.mapping?.parameterInputs || []).length > 0 && (
                            <div className="flex flex-wrap gap-x-4 gap-y-2 mb-4 bg-white/[0.03] border border-white/10 p-3 rounded-xl overflow-x-auto max-h-32 no-scrollbar">
                                {workflows.find(w => w.id === selectedWorkflowId)?.mapping?.parameterInputs?.map((param, idx) => (
                                    <div key={idx} className="flex flex-col gap-1 min-w-[120px]">
                                        ...
                                        {param.type === "boolean" ? (
                                            <button onClick={() => setExtraParams(p => ({ ...p, [`${param.nodeId}-${param.field}`]: !p[`${param.nodeId}-${param.field}`] }))} className={cn("px-3 py-1.5 rounded-lg border text-[9px] font-bold uppercase transition-all", extraParams[`${param.nodeId}-${param.field}`] ? "bg-primary/20 border-primary text-primary" : "border-white/10 text-muted-foreground bg-white/5")}>{extraParams[`${param.nodeId}-${param.field}`] ? "On" : "Off"}</button>
                                        ) : (
                                            <input type={param.type === "number" ? "number" : "text"} value={extraParams[`${param.nodeId}-${param.field}`] ?? ""} onChange={e => setExtraParams(p => ({ ...p, [`${param.nodeId}-${param.field}`]: param.type === "number" ? parseFloat(e.target.value) : e.target.value }))} className="bg-black/50 border border-white/10 rounded-lg px-3 py-1.5 text-[9px] font-bold text-white focus:ring-1 focus:ring-primary outline-none" />
                                        )}
                                    </div>
                                ))}
                            </div>
                        ...
                        <div className="relative flex items-center">
                            <textarea
                                ...
                                rows={2}
                                className="w-full bg-[#0c0c0e] border border-white/10 hover:border-white/20 rounded-2xl pl-4 pr-16 py-4 text-sm text-white focus:ring-1 focus:ring-primary outline-none resize-none no-scrollbar shadow-xl transition-colors disabled:opacity-50"
                                placeholder={workflows.find(w => w.id === selectedWorkflowId)?.mapping?.promptPlaceholder || "Descreva a foto..."}
                                disabled={history.some(h => h.isLoading)}
                            />
                            <button onClick={handleGenerate} disabled={history.some(h => h.isLoading)} className="absolute right-3 top-1/2 -translate-y-1/2 w-10 h-10 bg-primary/90 text-white rounded-xl flex items-center justify-center hover:bg-primary transition-all hover:scale-105 shadow-xl">
                                {history.some(h => h.isLoading) ? <Loader2 className="w-4 h-4 animate-spin" /> : <Sparkles className="w-4 h-4" />}
                            </button>
                        </div>
                        {errorMsg && <p className="text-[9px] text-destructive font-bold uppercase tracking-widest mt-2">{errorMsg}</p>}
                    </div>
                </div>
            </div>
