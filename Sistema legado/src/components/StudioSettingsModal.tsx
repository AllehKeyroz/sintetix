...
export function StudioSettingsModal({ isOpen, onClose, workflows, setWorkflows, isAdminMode = false }: StudioSettingsModalProps) {
    ...
    const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        reader.onload = (event) => {
            ...
            if (content) {
                // Se o JSON vir carregado mas o nome estava vazio, preenche o nome com o do arquivo
    ...
    const handlePastJson = (jsonString: string) => {
        try {
            ...
            // Handle Nordy's inner 'output' format
    ...
    const saveWorkflow = async () => {
        ...
        const workflowData: Partial<ComfyWorkflow> = {
            ...
            nordyWorkflowId: editingWorkflow.nordyWorkflowId || "",
        ...
        try {
            if (isAdminMode) {
                // Salvar no Firebase
                const savedId = await saveWorkflowTemplate(workflowData as ComfyWorkflow);
                    ...
                    {view === "edit" && editingWorkflow && (
                        <div className="space-y-4">
                            ...
                            <div className="space-y-1.5">
                                <label className="text-[9px] uppercase font-bold text-muted-foreground tracking-widest ml-1">Nordy Workflow ID (Execution ID)</label>
                                <input
                                    type="text"
                                    value={editingWorkflow.nordyWorkflowId || ""}
                                    onChange={e => setEditingWorkflow({ ...editingWorkflow, nordyWorkflowId: e.target.value })}
                                    className="w-full bg-background border border-white/10 rounded-xl px-4 py-3 text-sm font-bold text-white focus:ring-1 focus:ring-primary outline-none"
                                    placeholder="Obtenha no Nordy -> Settings -> Workflows"
                                />
                            </div>
                                        ...
                                        <div className="space-y-1.5">
                                            ...
                                            >
                                                ...
                                                {Object.entries(parsedNodes).map(([id, node]) => {
                                                    // Handle Nordy's nested outputs format if it exists, else assume flat Comfy format
